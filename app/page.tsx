...
import { ChevronRight, AlertTriangle, TrendingUp, Clock,
FileText, DollarSign, Shield, Zap, CircleDashed, ArrowRight, Brain, Sparkles,
AlertCircle, CheckCircle, X, Activity, Bell, BarChart3, Target } from
'lucide-react';
...
function appReducer(state, action) {
  switch (action.type) {
    case 'START_DEMO':
      return { ...state, loading: true, demoHasRun: false, conflicts: [], opportunities: [] };
    case 'LOAD_DATA':
    ...
    case 'UPDATE_COUNTERS':
      ...
      // Update only live counter if demo hasn't run
       return { 
           ...state,
           stats: {
             ...state.stats,
...
const ParticleField = () => {
  ...
  useEffect(() => {
    ...
    // Defer particle animation to avoid blocking initial render
...
const ResolutionModal = ({ conflict, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-slate-900/90 backdrop-blur-xl border border-amber-500/30 rounded-3xl p-8 w-full max-w-lg transform animate-scale-in shadow-2xl shadow-amber-500/20">
                ...
                <p className="text-slate-400 mb-6">{conflict.description}</p>
                ...
                <button onClick={onClose} className="w-full mt-8 bg-slate-800 text-white py-3 rounded-xl font-medium hover:bg-slate-700 transition-colors">
                    Close
                </button>
            </div>
        </div>
...
const DashboardCard = ({ title, value, unit, icon: Icon, color, isVisible, delay = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-slate-800/50 group-hover:border-slate-700 transition-all h-full">
      ...
      <div className="relative">
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            ...
            <div className="mt-3 flex items-center gap-2 text-xs text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const notificationInterval = setInterval(() => {
      if (!state.demoHasRun) return; // Only show notifications after demo runs
      const notifications = [
  ...
  const handleSelectConflict = useCallback((conflict) => {
    dispatch({ type: 'SELECT_CONFLICT', payload: conflict });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 font-sans overflow-x-hidden">
      ...
      <div className="fixed top-24 right-6 z-40 space-y-3 max-w-sm w-full">
      ...
      {state.selectedConflict && <ResolutionModal conflict={state.selectedConflict} onClose={handleCloseModals} />}
      ...
      <header className="relative z-10 text-center py-24 md:py-32 px-6">
        ...
        <AnimatedText delay={400}>
          <p className="text-xl md:text-2xl text-slate-400 max-w-3xl mx-auto mb-10">
            Every context window, a new amnesia. Foldera remembers, detects, and fixes costly mistakes <span className="text-white font-medium"> while you sleep.</span>
          </p>
        </AnimatedText>
      ...
      <section className="py-12 border-y border-slate-800/50">
        <div className="max-w-6xl mx-auto px-6 text-center">
            <p className="text-sm text-slate-400">
                <span className="font-semibold text-white">Built for high-stakes operators</span> in consulting, finance, and legal teams.
            </p>
        </div>
      </section>

      <main ref={mainRef} className="relative z-10 max-w-7xl mx-auto px-6 py-24">
        <div className="text-center mb-16">
          ...
          <AnimatedText delay={200}>
            <h2 className="text-4xl md:text-5xl font-light text-white mb-6">
              Three Landmines. Two Opportunities.
        ...
        {state.loading ? (
          <div className="flex flex-col items-center justify-center min-h-[50vh]">
            ...
            <p className="mt-2 text-slate-400 text-sm">This is a demo simulation</p>
          </div>
        ) : !state.demoHasRun ? (
            <div className="text-center min-h-[50vh] flex flex-col items-center justify-center">
                ...
                <p className="text-slate-400 mb-8">Click "Run Free Scan" above to start the live demo.</p>
            </div>
        ) : (
          <div className="bg-slate-900/30 backdrop-blur-xl border border-slate-800/50 rounded-3xl p-8 shadow-2xl animate-fade-in">
      ...
      <section className="py-24 px-6 text-center">
        <AnimatedText>
          ...
          <p className="text-xl text-slate-400 max-w-2xl mx-auto mb-10">
            Join {state.stats.liveCounter.toLocaleString()} professionals who've upgraded to AI that actually works.
          </p>
          <button 
            onClick={handleAuthAction}
